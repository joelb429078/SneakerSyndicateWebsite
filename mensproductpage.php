<?php
require_once 'connect.php'; // Imports database connection code

if (isset($_GET['ProductID'])) { // Checks if a 'ProductID' has been sent in the GET request
    $ProductID = mysqli_real_escape_string($conn, $_GET['ProductID']); // Retrieves and sanitizes the 'ProductID' value from the GET request using mysqli_real_escape_string() function

    $SQL = "SELECT * FROM product WHERE ProductID='$ProductID'"; // SQL query to select all data from the 'product' table where the 'ProductID' matches the value passed in the GET request
    $result = mysqli_query($conn, $SQL) or die("Bad Query: $SQL"); // Executes the query and stores the result in the '$result' variable. If the query fails, it stops the script and prints an error message
    $row = mysqli_fetch_array($result); // Fetches the first row from the query result and stores it in the '$row' variable

} else { // If no 'ProductID' is found in the GET request, redirect to 'mensproductsog.php' page
    header('Location: mensproductsog.php');
}
?>
<?php
